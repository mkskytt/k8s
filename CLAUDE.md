...
- **Pinning vs. ranges**: secret-handling charts (`external-dns`, `cloudflare-tunnel`) pin exact versions; supporting infrastructure (`cnpg`, `kyverno`, `grafana-k8s-monitoring`) uses `>=` ranges. Follow the existing pattern of the app you're touching rather than changing it.
